...
#initialize the keypoint detector, loccal invariant descriptor and the descriptor pipeline
...
random.shuffle(imagePaths) #this will make easier for train/test split

#loop over the images in the dataset
for (i, imagePath) in enumerate(imagePaths):
    #check to see if the progress should be displayed
    if i>0 and i%10==0:
        fi._debug("processed {} images".format(i), msgType="[PROGRESS]")

    #extract the filename and the image class from the image path and use it to construct the unique image ID
    p = imagePath.split("/")
