...
#handle sklearn version less then 0.18
...
#oprn the features and bag of visual words databases
featuresDB = h5py.File(args["features_db"])
bovwDB = h5py.File(args["bovw_db"])

#grab training and testing data from the dataset using the first 300 images as training and remaining 200 images for testing
...
model = GridSearchCV(LinearSVC(random_state=42, max_iter=100000), params, cv=3)
...
#runing this file
#python train_model.py --dataset dataset/caltech5 --features-db output/features.hdf5 --bovw-db output/bovw.hdf5 --model output/model.cpickle  